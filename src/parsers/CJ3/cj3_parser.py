...
from handlers import CJ3AudioHandler
from parsers.CJ3.cj3_utils import CJ3Utils


class CJ3Parser(YomitanParser):
    ...
    def _handle_missing_entry_keys(self, soup: bs4.BeautifulSoup) -> int:
        ...
        pinyin_readings = CJ3Parser.process_readings(pinyin_readings)
    ...
    @staticmethod
    def process_readings(pinyin_readings: List[str]) -> List[str]:
        processed_readings = []
    ...
    def _process_file(self, filename: str, file_content: str) -> int:
        ...
        if len(hanzi_keys) == 2 and len(original_readings) == 1 and '(' in original_readings[0] and ')' in original_readings[0]:
            ...
            processed_readings = CJ3Parser.process_readings(original_readings[:])
