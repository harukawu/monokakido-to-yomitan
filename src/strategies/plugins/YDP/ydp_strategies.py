...
class YDPImageHandlingStrategy(DefaultImageHandlingStrategy):
    def __init__(self):
        self.replacements = {
            "arrow-thick.svg": {
                "text": "➡",
        ...
        self.dictionary_path = Path(__file__).parent.parent.parent.parent.parent / "resources/YDP/assets"
    ...
    def extract_svg_viewbox(self, svg_path: str) -> Tuple[float, float]:
        ...
        try:
            ...
            full_path = self.dictionary_path.joinpath(svg_path)
    ...
    def calculate_svg_dimensions(self, svg_path: str) -> Dict:
        ...
        equation_type = YDPImageHandlingStrategy.classify_svg_content(width, height)
        ...
        if equation_type in ("wide_equation", "tall_wide_equation") and config["display_as_block"]:
            ...
            # Only apply if the equation is wider than target
    ...
    @staticmethod
    def classify_svg_content(width: float, height: float) -> str:
    ...
    def handle_image_element(self, html_glossary: bs4.element.Tag, html_elements: List, data_dict: Dict,
                             class_list: List[str]) -> Dict: # TODO: change to optional
        img_path = html_glossary.get("src", "").lstrip("/")

        if not img_path or re.search(r'\.eps', img_path, flags=re.IGNORECASE):
            return None

        #if "P0029_01" in img_path or "P0029_02" in img_path:
            #print("break")
